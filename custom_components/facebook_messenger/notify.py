...
def get_service(hass, config, discovery_info=None):
    ...
    return FacebookNotificationService(
        config[CONF_PAGE_ACCESS_TOKEN],
        config.get(CONF_TARGETS, [])
    )
    ...
    def send_message(self, message="", **kwargs):
        ...
        for target in targets:
            ...
            try:
                if media:
                    with open(media, "rb") as media_file:
                        resp = requests.post(
                            ...
                            files={"filedata": ("media.jpg", media_file, media_type)},
                            timeout=10,
                        )
                else:
                    body = {
                ...
                _LOGGER.error(f"Failed to send message to {target}: {e}")


def log_error(response):
    """Log error message from Facebook API."""
    try:
        obj = response.json()
